...
# 파일 경로
file_path = 'C:/python_ex/jumpit_data.json'  # JSON 파일의 경로를 지정
...
try:
    for item in data:
        # JOB_POSTING 테이블에 데이터 삽입
        cursor.execute("""
            INSERT INTO JOB_POSTING (COMPANY, TITLE, RECRUIT_DEADLINE, WORKING_AREA, JOB_POSTING_URL, JOB_GROUP, 
                       GROUP_INTRO, MAINDUTIES, QUALIFICATION, PREFERENTIAL, BENEFITS, TITLE_IMG, LOGO_IMG)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (item['COMPANY'],
              item['TITLE'], 
              item.get('RECRUIT_DEADLINE'), 
              item['WORKING_AREA'], 
